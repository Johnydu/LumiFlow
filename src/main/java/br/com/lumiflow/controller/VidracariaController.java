...
public class VidracariaController {
    ...
    public String registrarMovimentacao(@Valid @ModelAttribute("movimentacaoDTO") MovimentacaoVidroDTO dto, BindingResult result, Model model, RedirectAttributes attributes, Locale locale) {
        if (result.hasErrors()) return erroValidacao(model, locale);
        vidracariaService.registrarMovimentacao(dto);
        attributes.addFlashAttribute("success", messageSource.getMessage(AppMessages.SUCCESS_GLASS_MOVEMENT_CREATED, null, locale));
        return "redirect:/dashboard/vidracaria";
    }

    @PostMapping("/cadastrar-chapa")
    public String cadastrarChapa(@Valid @ModelAttribute("chapaDTO") ChapaVidroDTO dto, BindingResult result, Model model, RedirectAttributes attributes, Locale locale) {
        if (result.hasErrors()) return erroValidacao(model, locale);
        vidracariaService.cadastrarChapa(dto);
        attributes.addFlashAttribute("success", messageSource.getMessage(AppMessages.SUCCESS_GLASS_TYPE_CREATED, null, locale));
        return "redirect:/dashboard/vidracaria";
    }
